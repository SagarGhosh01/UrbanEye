...
import { Camera, Zap, Smartphone, Layers, Crosshair } from 'lucide-react';
  ...
  return (
    <div className="bg-slate-900 rounded-xl border border-slate-800 p-4 shadow-lg flex flex-col h-full">
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-100 uppercase tracking-wide flex items-center space-x-2">
              <span>Edge AI Vision Stream</span>
              <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono border ${
                livePhoneFrame ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30 animate-pulse' : 'bg-sky-500/20 text-sky-400 border-sky-500/30'
              }`}>
                {livePhoneFrame ? 'REAL PHONE CAM ACTIVE' : 'SIMULATED FRONT CAM'}
              </span>
            </h3>
            <p className="text-xs text-slate-400">
              YOLOv8n Multi-Object Tracking • EasyOCR • Real Pothole CV
            </p>
          </div>
        </div>

        {/* Bus Selector & Phone Mode Button */}
        <div className="flex items-center space-x-2">
          {onOpenPhoneMode && (
            <button
              onClick={onOpenPhoneMode}
              className="px-2.5 py-1.5 bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white rounded-lg text-xs font-semibold flex items-center space-x-1.5 shadow-sm transition-all"
            >
              <Smartphone className="w-3.5 h-3.5" />
              <span>Use Phone as Cam</span>
            </button>
          )}
          ...
          <select
            ...
            className="bg-slate-950 border border-slate-700 text-xs text-slate-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-sky-500 font-mono cursor-pointer"
          >
            {buses.map((b) => (
              <option key={b.bus_id} value={b.bus_id}>
                {b.bus_id} ({b.registration_no || 'Bus Unit'})
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="relative mt-3 rounded-lg overflow-hidden bg-slate-950 border border-slate-800 aspect-video flex items-center justify-center group shadow-inner">
        ...
        {/* Top HUD Overlay */}
        <div className="absolute top-2 left-2 right-2 flex items-center justify-between text-[11px] font-mono text-slate-300 bg-slate-950/80 backdrop-blur-sm px-3 py-1.5 rounded border border-slate-800">
          <div className="flex items-center space-x-3">
            <span className="text-emerald-400 font-bold flex items-center">
              <span className="w-2 h-2 rounded-full bg-emerald-500 mr-1.5 animate-ping" />
              {livePhoneFrame ? 'PHONE LINK' : 'REC'}
            </span>
            <span>BUS: <strong className="text-white">{selectedBusId}</strong></span>
            <span>CAM: <strong className="text-sky-400">{livePhoneFrame ? 'PHONE_FRONT' : selectedCam}</strong></span>
          </div>
          <div className="flex items-center space-x-4">
            <span>FPS: <strong className="text-emerald-400">{fps}</strong></span>
            <span>LATENCY: <strong className="text-amber-400">{latencyMs}ms</strong></span>
            <span>ACCEL: <strong className="text-slate-200">YOLOv8 Real Inference</strong></span>
          </div>
        </div>

        {/* Bottom Camera Position Selectors */}
        <div className="absolute bottom-2 left-2 flex items-center space-x-1.5 bg-slate-950/90 backdrop-blur-sm p-1 rounded-lg border border-slate-800">
          {['FRONT', 'REAR', 'LEFT', 'RIGHT'].map((pos) => (
            <button
              ...
              className={`px-2.5 py-1 text-[10px] font-bold rounded font-mono transition-all ${
        ...
        <div className="absolute bottom-2 right-2 flex items-center space-x-1.5 bg-slate-950/90 backdrop-blur-sm p-1 rounded-lg border border-slate-800">
          <button
            onClick={() => setShowBoxes(!showBoxes)}
            className={`px-2 py-1 text-[10px] font-semibold rounded flex items-center space-x-1 ${
      ...
      <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-slate-800 text-xs">
        <div className="bg-slate-950/60 p-2 rounded-lg border border-slate-800/80">
          ...
          <span className="font-bold text-slate-100 font-mono">
            {livePhoneData ? `${livePhoneData.detections?.length || 0} Objects Detected` : '6 Vehicles • 2 Pedestrians'}
          </span>
        </div>
        <div className="bg-slate-950/60 p-2 rounded-lg border border-slate-800/80">
          <span className="text-slate-400 block text-[10px]">Road Hazard State</span>
          <span className="font-bold text-amber-400 font-mono">
            {livePhoneData ? `${livePhoneData.hazards?.length || 0} Potholes Found` : '1 Pothole • 1 Waterlogging'}
          </span>
        </div>
        <div className="bg-slate-950/60 p-2 rounded-lg border border-slate-800/80">
          <span className="text-slate-400 block text-[10px]">Scanned ANPR Plate</span>
